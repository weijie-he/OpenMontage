...
| **TTS & Audio** | `text-to-speech`, `speech-to-text` (whisper, default STT), `azure-speech-to-text` (optional cloud STT), `music`, `sound-effects`, `elevenlabs`, `agents`, `setup-api-key`, `minimax` | `elevenlabs/skills`, `digitalsamba/claude-code-video-toolkit`, local OpenMontage skill |
...
| **MiniMax Direct** | `minimax` — official Hailuo 2.3 video plus Speech 2.8 TTS under `MINIMAX_API_KEY` | Local OpenMontage skill |
